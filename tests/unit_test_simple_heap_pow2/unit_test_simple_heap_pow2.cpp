...
bool validate_allocation(HeapType* heap, std::size_t allocation_size, std::size_t logical_page_size, std::vector<Allocation>& pointer_vector, std::size_t repeat_count = 1)
{
    for (std::size_t i = 0; i < repeat_count; i++)
    {
        ...
        else
        {
            ...
            {
                std::size_t size_adjusted = Pow2Utilities::get_first_pow2_of(allocation_size);
                size_adjusted = size_adjusted < HeapType::MIN_SIZE_CLASS ? HeapType::MIN_SIZE_CLASS : size_adjusted;

                bool is_page_header_good = false;
                is_page_header_good = size_adjusted == HeapType::SegmentType::get_size_class_from_address(ptr, logical_page_size);
...
bool validate_aligned_allocation(HeapType* heap, std::size_t allocation_size, std::size_t alignment, std::vector<Allocation>& pointer_vector, std::size_t repeat_count = 1)
{
    for (std::size_t i = 0; i < repeat_count; i++)
    {
        ...
        else
        {
            ...
            {
                std::cout << "ALIGNMENT CHECK FAILED , alloc size = " << allocation_size << " alignment = " << alignment << std::endl;
                return false;
            }
...
{
    std::vector<std::size_t> size_classes = { 16,32,64,128,256,512,1024,2048, 4096, 8192, 16384, 32768 };
    ...
    // HEAP CREATION FAILURE
    {
        ...
        params.m_logical_page_size = 65500; // Wrong param intended
    ...
    {
        auto bin16_index = SizeUtilities::get_pow2_bin_index_from_size<SimpleHeapPow2<>::MIN_SIZE_CLASS, SimpleHeapPow2<>::MAX_BIN_INDEX>(16);
        auto bin32_index = SizeUtilities::get_pow2_bin_index_from_size<SimpleHeapPow2<>::MIN_SIZE_CLASS, SimpleHeapPow2<>::MAX_BIN_INDEX>(32);
        auto bin64_index = SizeUtilities::get_pow2_bin_index_from_size<SimpleHeapPow2<>::MIN_SIZE_CLASS, SimpleHeapPow2<>::MAX_BIN_INDEX>(64);
        auto bin128_index = SizeUtilities::get_pow2_bin_index_from_size<SimpleHeapPow2<>::MIN_SIZE_CLASS, SimpleHeapPow2<>::MAX_BIN_INDEX>(128);
        auto bin256_index = SizeUtilities::get_pow2_bin_index_from_size<SimpleHeapPow2<>::MIN_SIZE_CLASS, SimpleHeapPow2<>::MAX_BIN_INDEX>(256);
        auto bin512_index = SizeUtilities::get_pow2_bin_index_from_size<SimpleHeapPow2<>::MIN_SIZE_CLASS, SimpleHeapPow2<>::MAX_BIN_INDEX>(512);
        auto bin1024_index = SizeUtilities::get_pow2_bin_index_from_size<SimpleHeapPow2<>::MIN_SIZE_CLASS, SimpleHeapPow2<>::MAX_BIN_INDEX>(1024);
        auto bin2048_index = SizeUtilities::get_pow2_bin_index_from_size<SimpleHeapPow2<>::MIN_SIZE_CLASS, SimpleHeapPow2<>::MAX_BIN_INDEX>(2048);
    ...
    ///////////////////////////////////////////////////////////////////////////////////////
    // HEAP GENERAL TESTS , WHEN LOGICAL PAGE SIZE IS 64KB
    {
        SimpleHeapPow2<> heap;
        Arena<> arena;
        bool success = arena.create(65536 * 10, 65536);
        ...
        std::size_t object_capacity_per_size_class = 1024 * 16;
        params.m_logical_page_recycling_threshold = object_capacity_per_size_class * 2;
        auto counter = 0;

        for (const auto& size_class : size_classes)
        {
            params.m_bin_logical_page_counts[counter] = (object_capacity_per_size_class * size_class) / 65536;
            counter++;
        }
        ...
        std::vector<Allocation> object_allocations;

        for (const auto& size_class : size_classes)
        {
            for (std::size_t i = 0; i < object_capacity_per_size_class; ++i)
            {
                bool success = validate_allocation(&heap, size_class, params.m_logical_page_size, object_allocations);
        ...
        unit_test.test_equals(object_allocations.size(), object_capacity_per_size_class * size_classes.size(), "heap pow 2", "verifying allocation count");

        // VERIFY LOGICAL PAGE COUNTS BEFORE GROWING IN SIZE
        unit_test.test_equals(heap.get_bin_logical_page_count(0), 5, "heap pow 2", "Bin16   logical page count");
        unit_test.test_equals(heap.get_bin_logical_page_count(1), 9, "heap pow 2", "Bin32   logical page count");
        unit_test.test_equals(heap.get_bin_logical_page_count(2), 17, "heap pow 2", "Bin64   logical page count");
        unit_test.test_equals(heap.get_bin_logical_page_count(3), 33, "heap pow 2", "Bin128  logical page count");
        unit_test.test_equals(heap.get_bin_logical_page_count(4), 65, "heap pow 2", "Bin256  logical page count");
        unit_test.test_equals(heap.get_bin_logical_page_count(5), 130, "heap pow 2", "Bin512  logical page count");
        unit_test.test_equals(heap.get_bin_logical_page_count(6), 261, "heap pow 2", "Bin1024 logical page count");
        unit_test.test_equals(heap.get_bin_logical_page_count(7), 529, "heap pow 2", "Bin2048 logical page count");

        // TRIGGER GROWING IN OBJECT BINS THEN VALIDATE LOGICAL PAGE COUNTS
        if (validate_allocation(&heap, 16, params.m_logical_page_size, object_allocations, object_capacity_per_size_class) == false) return -1;
        if (validate_allocation(&heap, 32, params.m_logical_page_size, object_allocations, object_capacity_per_size_class) == false) return -1;
        if (validate_allocation(&heap, 64, params.m_logical_page_size, object_allocations, object_capacity_per_size_class) == false) return -1;
        if (validate_allocation(&heap, 128, params.m_logical_page_size, object_allocations, object_capacity_per_size_class) == false) return -1;
        if (validate_allocation(&heap, 256, params.m_logical_page_size, object_allocations, object_capacity_per_size_class) == false) return -1;
        if (validate_allocation(&heap, 512, params.m_logical_page_size, object_allocations, object_capacity_per_size_class) == false) return -1;
        if (validate_allocation(&heap, 1024, params.m_logical_page_size, object_allocations, object_capacity_per_size_class) == false) return -1;
        if (validate_allocation(&heap, 2048, params.m_logical_page_size, object_allocations, object_capacity_per_size_class) == false) return -1;

        unit_test.test_equals(heap.get_bin_logical_page_count(0), 9, "heap pow 2", "Bin16   logical page coun post growth");
        unit_test.test_equals(heap.get_bin_logical_page_count(1), 17, "heap pow 2", "Bin32   logical page count post growth");
        unit_test.test_equals(heap.get_bin_logical_page_count(2), 33, "heap pow 2", "Bin64   logical page count post growth");
        unit_test.test_equals(heap.get_bin_logical_page_count(3), 65, "heap pow 2", "Bin128  logical page count post growth");
        unit_test.test_equals(heap.get_bin_logical_page_count(4), 129, "heap pow 2", "Bin256  logical page count post growth");
        unit_test.test_equals(heap.get_bin_logical_page_count(5), 259, "heap pow 2", "Bin512  logical page count post growth");
        unit_test.test_equals(heap.get_bin_logical_page_count(6), 521, "heap pow 2", "Bin1024 logical page count post growth");
        unit_test.test_equals(heap.get_bin_logical_page_count(7), 1058, "heap pow 2", "Bin2048 logical page count post growth");

        // DEALLOCATIONS
        for (const auto object_allocation : object_allocations)
        {
            auto size_class = SimpleHeapPow2<>::SegmentType::get_size_class_from_address(object_allocation.ptr, params.m_logical_page_size);

            if (size_class != object_allocation.size_class)
            {
                std::cout << "PAGE HEADER VALIDATION FAILED : FOUND SIZECLASS IS WRONG !!!" << std::endl;
                return -1;
            }

            heap.deallocate(reinterpret_cast<void*>(object_allocation.ptr));
        }
    }
    ...
    // ALIGNED ALLOCATIONS
    {
        ...
        params.m_logical_page_recycling_threshold = 4096;
    ...
    ///////////////////////////////////////////////////////////////////////////////////////
    // DEALLOCATIONS FOR ALIGNED ALLOCATIONS : SHOULD ADD CORRECT PTRS BACK TO THE LOGICAL PAGES
    {
        SimpleHeapPow2<> heap;
        Arena<> arena;
        bool success = arena.create(65536 * 10, 65536);
        ...
        void* first_ptr = heap.allocate_aligned(5, 32);
        heap.deallocate(first_ptr);

        for (std::size_t i = 0; i < 100; i++)
        {
            auto ptr = heap.allocate_aligned(5, 32);

            if (ptr != first_ptr)
            {
                std::cout << "TEST FAILED, DEALLOCATION AFTER ALIGNED ALLOCATIONS ARE NOT ADDING CORRECT POINTERS BACK TO THE UNDERLYING LOGICAL PAGES !!!\n";
                return -1;
            }
    ...
    // BOUNDED HEAP
    {
        ...
        for (const auto& size_class : size_classes)
        {
            params.m_bin_logical_page_counts[counter] = SizeUtilities::get_required_page_count_for_allocation(params.m_logical_page_size, sizeof(LogicalPage<>), size_class, object_capacity_per_size_class);
            counter++;
        }
        ...
        unit_test.test_equals(object_allocations.size(), object_capacity_per_size_class* size_classes.size(), "heap pow 2", "verifying object allocation count");

        // VERIFY OBJECT LOGICAL PAGE COUNTS
        unit_test.test_equals(heap.get_bin_logical_page_count(0), 5, "heap pow 2", "Bin16   logical page count");
        unit_test.test_equals(heap.get_bin_logical_page_count(1), 9, "heap pow 2", "Bin32   logical page count");
        unit_test.test_equals(heap.get_bin_logical_page_count(2), 17, "heap pow 2", "Bin64   logical page count");
        unit_test.test_equals(heap.get_bin_logical_page_count(3), 33, "heap pow 2", "Bin128  logical page count");
        ...
        for (const auto object_allocation : object_allocations)
        {
            auto size_class = TestHeapType::SegmentType::get_size_class_from_address(object_allocation.ptr, params.m_logical_page_size);
    ...
    // RANDOM ALLOCATIONS
    {
        ...
        {
            auto size_class = SimpleHeapPow2<>::SegmentType::get_size_class_from_address(allocation.ptr, params.m_logical_page_size);

            std::size_t size_adjusted = Pow2Utilities::get_first_pow2_of(allocation.size_class);
            size_adjusted = size_adjusted < SimpleHeapPow2<>::MIN_SIZE_CLASS ? SimpleHeapPow2<>::MIN_SIZE_CLASS : size_adjusted;

            if (size_class != size_adjusted)
            {
