...
[[nodiscard]] bool start(const std::string& address, int port)
{
    return g_http_live_profiler.start(address, port);
}
...
void capture_custom_allocation(void* address, std::size_t size)
{
    Profiler::get_instance().capture_allocation(address, size);
}

void capture_custom_deallocation(void* address)
{
    Profiler::get_instance().capture_deallocation(address);
}
